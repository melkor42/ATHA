...
- **Perspective is the primary persona axis.** enterprise / talent / infrastructure / education — inherited from the four tabs of the page the Wow Page replaces (signal.onemundi.one). It selects the section mix and the spectrum.
- **Design is an axis grammar, not themes.** The retired boardroom / festival / garden / ledger theme enum gave way to two orthogonal axes: Spectrum (mycelium | terra | aurora | neon | void, perspective-driven) owns the hue family; StyleMode (none | minimal | retro | organic | earth | steampunk, design-words/temperament-driven) owns materiality, typography and atmosphere. Both map to frontend CSS variable sets; the LLM selects only the enums.
