...
- **Deploying the code does not deploy its configuration.** `install-vm.sh` deliberately never touches an existing `/opt/atha/repo/.env`, and `backend/agents.py` resolves the provider (`_IS_CEREBRAS` / `_IS_GROQ`) plus both model rungs at *import* time — so `LLM_BASE_URL`, `LLM_API_KEY`, `PRIMARY_MODEL`, `FALLBACK_MODEL` and `LLM_MAX_TOKENS` must move together, and only once the matching code is on the box. Mirror them with `./.scratch/env-vm.sh` (`RESTART=1` to restart `atha` after merging, `ROLLBACK=1` to restore the newest timestamped backup); it prints key names and non-secret values only. Skipping this is how the VM was found still composing through Groq on a 1400-token default after `main` moved to Cerebras on 2026-09-04.
...
- Against a running host, `eval_loop.py --url <host>` twice in a row reports **cache-hit** latency (3–50 ms): `backend/main.py` caches responses by `role|visitor_state|intent|facet|free_text|style`. `topics` is **not** in that key and is ignored outright, so varying it does not force a compose — vary `free_text`, or start a second instance on its own port for a genuinely cold process.
