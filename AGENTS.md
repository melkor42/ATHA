...
- `townsquare.de` is meant to be the VM's name, but as of 2026-09-04 it resolves to `195.201.235.173` (a Hetzner box whose Caddy answers `:80` with `302 → https` and refuses `:443`) — so it reaches **neither** host and no TLS terminates. Use the IP or the Render URL when verifying; don't assume the domain works.
